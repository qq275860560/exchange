...
task_invoke_type  tinyint(1) unsigned not null DEFAULT 0 comment '任务调用类型,0:远程调用，需要设置http请求链接，请求方法，请求头部，请求体json字符串,1:本地调用,需要设置spring容器中的类全名，方法名称，方法输入参数DTO类型全名(只支持一个DTO)，方法输入参数DTO对应的json字符串',
...
request_url varchar(256)  DEFAULT NULL COMMENT 'httpUrl链接，局域网内连接，内部服务连接，通常是微服务链接',
request_method varchar(8)  DEFAULT NULL COMMENT '请求方法(GET/POST/PUT/DELETE)，默认为POST',
request_header longtext DEFAULT NULL COMMENT '请求头部json格式',
input_json longtext   DEFAULT NULL COMMENT '请求参数json,请求体内容 ',
task_class_name  varchar(256)  DEFAULT NULL COMMENT '类全名',
task_method_name  varchar(256)  DEFAULT NULL COMMENT '方法名称',
task_parameter_class_name  varchar(256)  DEFAULT NULL COMMENT '方法输入参数DTO类型全名(只支持一个DTO)',
task_parameter_json  varchar(256)  DEFAULT NULL COMMENT '方法输入参数DTO对应的json字符串',
...
task_invoke_type  tinyint(1) unsigned not null DEFAULT 0 comment '任务调用类型,0:远程调用，需要设置http请求链接，请求方法，请求头部，请求体json字符串,1:本地调用,需要设置spring容器中的类全名，方法名称，方法参数DTO类型全名(只支持一个DTO)，方法参数DTO对应的json字符串',
...
task_parameter_class_name  varchar(256)  DEFAULT NULL COMMENT '方法输入参数DTO类型全名(只支持一个DTO)',
task_parameter_json  varchar(256)  DEFAULT NULL COMMENT '方法输入参数DTO对应的json字符串',
task_return_json varchar(256)  DEFAULT NULL COMMENT '方法输出参数DTO对应的json字符串',
